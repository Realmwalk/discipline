...
This is Discipline's own project-local agent playbook. It annotates only Discipline's divergences from the framework defaults for working ON Discipline itself; the canonical templates ship under `templates/` (what `npx discipline init` installs).
...
6. Run `npx discipline lint` (here: `node bin/discipline.js lint`) and fix anything red before signing off.
